...
namespace QuickCross
{
	...
	{
		/// <summary>
		/// Gets the name of a property or field in a typesafe manner
		/// </summary>
		/// <param name="expression">
		/// A Linq expression that specifies the name of a property or field in a typesafe manner
		/// <example>() => obj.AProperty</example>
		/// <example>() => obj.AField</example>
		/// </param>
		...
		public object Value
		{
			get
			{
				if (propertyInfo != null) return propertyInfo.GetValue(ContainingObject);
				if (fieldInfo != null) return fieldInfo.GetValue(ContainingObject);
				throw new InvalidOperationException("Cannot access the Value because no PropertyOrFieldName was specified");
			}

			set
			{
				if (Value == value) return;
				if (propertyInfo != null) propertyInfo.SetValue(ContainingObject, value);
				else fieldInfo.SetValue(ContainingObject, value);
			}
		}

		public PropertyReference(object containingObject, string propertyOrFieldName)
		{
		...
		/// <example>() => obj.AField</example>
		/// </param>
		public PropertyReference(object containingObject, Expression<Func<object>> propertyOrField)
			: this(containingObject, GetMemberName(propertyOrField))
		{ }
	}
}